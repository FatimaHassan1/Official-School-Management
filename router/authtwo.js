...
router.route('/getsingle/:reid').get((req , res)=>{
    ...
    studentModel.find({_id: req.params.reid})
...
router.route('/studentedit/:reid').put(async(req , res)=>{
    ...
    try{
        await studentModel.updateOne({_id : req.params.reid} , editStudent)
...
router.route('/approvestationary/:appid').put(async(req , res)=>{
    try{
        await stationaryModel.updateOne({_id : req.params.appid} , {$set : {status : "Approved"}})
        .then(updated => res.json(updated))
    }catch(error){
        console.log(error)
    }
...
router.route('/rejectstationary/:appid').put(async(req , res)=>{
    try{
        await stationaryModel.updateOne({_id : req.params.appid} , {$set : {status : "Rejected"}})
        .then(updated => res.json(updated))
    }catch(error){
        console.log(error)
    }
...
router.route('/approvepurchase/:appid').put(async(req , res)=>{
    try{
        await purchaseModel.updateOne({_id : req.params.appid} , {$set : {status : "Approved"}})
        .then(updated => res.json(updated))
    }catch(error){
        console.log(error)
    }
...
router.route('/rejectpurchase/:appid').put(async(req , res)=>{
    try{
        await purchaseModel.updateOne({_id : req.params.appid} , {$set : {status : "Rejected"}})
        .then(updated => res.json(updated))
    }catch(error){
        console.log(error)
    }
    ...
    try{
        async function auth() {
            ...
            let options = {
                ...
                adminDn: 'CN=huzafa,OU=test account,OU=Staff,OU=Domain Controllers,DC=IPS,DC=EDU',
